...
import cmath
import mpmath
from multi_X import multiscale_X
from utils import double_roughness_spectrum
import utils
from scipy.special import erfc

def backscatter(fr,sig1, L1, sig2, L2, thi, er): # Removed sig3, L3

        # sigma_0_vh = multiscale_X(fr, sig1, L1, sig2, L2, sig3, L3, thi, er)
        ...
        wn = double_roughness_spectrum(sig12, sig22, sigs2, L1, L2, k, s, Ts)

        rss = sqrt(2) / sigs2 * (sig12 * sig1 / L1 + sig22 * sig2 / L2)

        # ----------- compute R - transition - -----------

        Rv0 = (cmath.sqrt(er) - 1) / (cmath.sqrt(er) + 1)
        Rh0 = -Rv0

        Ft = 8 * Rv0 ** 2 * ss * (cs + cmath.sqrt(er - s2)) / (cs * cmath.sqrt(er - s2))
        a1 = 0
        b1 = 0
        for n in np.arange(1,Ts + 1):
                ...
                b1 = b1 + a0 * (abs(Ft / 2 + 2 ** (n + 1) *
                                    Rv0 / cs * math.exp(-(ks * cs) ** 2))) ** 2 *wn[n-1]
...
if __name__ == "__main__":
        ...
        er = complex(9, 3) # dielectric constant
        ...
        sigma_0_vv, sigma_0_hh = backscatter(fr, sig1, L1, sig2, L2, thi, er)
