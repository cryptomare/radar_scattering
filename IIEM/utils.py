...
def exponential_spectrum(z,wvnb,L,n,xx):
...
def spectrm2(sp, xx, kl2, L, rx, ry, s, np_, nr):
    ...
    if sp.lower() == 'math.exponential':  # math.exponential
...
def xpol_integralfunc_vec(r, phi, sp, xx, ks2, cs,s, kl2, L, er, rss, rvh, n_spec, factorials):
    ...
    #-- calculate math.expressions for the surface spectra
...
def multi_roughness_spectrum(sig12, sig22, sig32, sigs2, L1, L2, L3, k, s, Ts):

    wn = np.zeros(Ts, dtype=np.complex_)

    for n in np.arange(1,Ts+1):
        sum1 = 0
        for m in np.arange(1,n+1):
            sum2 = 0
            for p in np.arange(1,m+1):
                cg = n / L1 ** 2 + (1 / L2 ** 2 - 1 / L1 ** 2) * (m - p) + (1 / L3 ** 2 - 1 / L1 ** 2) * p

                sum2 = sum2 + math.factorial(m) / math.factorial(p) / math.factorial(m - p) \
                       *(sig22 / sig12) ** (m - p) * (sig32 / sig12) \
                       ** p * sqrt(math.pi / cg)*math.exp(- (2 * k * s) ** 2. / (4 * cg))

            sum1 = sum1 + math.factorial(n) / math.factorial(m) / math.factorial(n - m) * sum2

        wn[n-1] = (sig12 / sigs2) ** n * sum1

    return(wn)


def double_roughness_spectrum(sig12, sig22, sigs2, L1, L2, k, s, Ts):
    ...
    for n in np.arange(1,Ts+1):
        ...
        for m in np.arange(1,n+1):
            ...
            cg = (m*n) / L1 ** 2 + (1 / L2 ** 2 - 1 / L1 ** 2) * m

            sum1 = sum1 + (sig22 / sig12) ** (m) * sqrt(math.pi / cg)*math.exp(- (2 * k * s) ** 2. / (4 * cg))
